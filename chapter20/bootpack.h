...
#define TASK_GDT0     3     // TSS を GDT の何番から割り当てるか
#define MAX_TASKS_LV  100
#define MAX_TASKLEVES 10
struct TSS32 {
...
struct TASK {
  int sel, flags; // sel は GDT の番号のこと
  int level, priority;
  struct FIFO32 fifo;
  struct TSS32 tss;
};
struct TASKLEVEL {
  ...
  int now;      // 現在どうしているタスクがどれだけ変わるようにするための変数
  struct TASK *tasks[MAX_TASKS_LV];
};
struct TASKCTL {
  int now_lv;     // 現在動作中のレベル
  char lv_change; // 次回タスクスイッチのときにレベルも変えたほうがいいかどうか
  struct TASKLEVEL level[MAX_TASKLEVES];
...
void cmd_mem(struct CONSOLE *cons, int memman_total);
