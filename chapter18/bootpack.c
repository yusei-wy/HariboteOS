...
void HariMain(void) {
  ...
  io_out8(PIC0_IMR, 0xf8); // PIT と PTC1 とキーボードを許可
  ...
  *((int *) (task_cons->tss.esp + 4)) = (int) sht_cons;
  ...
  // 最初にキーボード状態との食い違いがないように設定しておくことにする
      ...
      if (256 <= i && i <= 511) { // キーボードデータ
        ...
        if (i == 256 + 0x0f) {  // Tab
          if (key_to == 0) {
            ...
            fifo32_put(&task_cons->fifo, 2);  // コンソールのカーソル ON
          } else {
            key_to = 0;
            make_wtitle8(buf_win, sht_win->bxsize,  "task_a", 1);
            make_wtitle8(buf_cons, sht_cons->bxsize, "console", 0);
            cursor_c = COL8_000000;  // カーソルを表示する
            fifo32_put(&task_cons->fifo, 3);  // コンソールのカーソル OFF
          }
        ...
        if (i == 256 + 0xfa)    // キーボードデータを無事に受け取った
          keycmd_wait = -1;
        if (i == 256 + 0xfe) {  // キーボードデータを無事に受け取れなかった
          wait_KBC_sendready();
          io_out8(PORT_KEYDAT, keycmd_wait);
        }
        // カーソルの再表示
...
void make_window8(unsigned char *buf, int xsize, int ysize, char *title, char act) {
  ...
  boxfill8(buf, xsize, COL8_000000, 0,          ysize - 1,  xsize - 1,  xsize - 1);
...
/**
 * 背景色を塗りつぶして、その上に文字を書いて、最後にリフレッシュする
 */
void putfonts8_asc_sht(struct SHEET *sht, int x, int y, int c, int b, char *s, int l) {
...
void console_task(struct SHEET *sheet) {
  ...
  int i, fifobuf[128], cursor_x = 16, cursor_c = -1;  // 起動時は task_a が入力状態になっているので
  char s[2];
