...
void HariMain(void) {
  ...
  sheet_slide(sht_win_b[2], 168, 166);
...
void task_b_main(struct SHEET *sht_win_b) {
  ...
  for (;;) {
    ...
    // NOTE: ここにで putfonts8_asc_sht を実行しないと動作しない
    sprintf(s, "task_b_main");
    putfonts8_asc_sht(sht_win_b, 0, 300, COL8_FFFFFF, COL8_008484, s, 11);
  }
}
